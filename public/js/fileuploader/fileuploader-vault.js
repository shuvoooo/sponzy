    ...
    var $fileuploader = $('input.gallery_media').fileuploader({
        ...
        thumbnails: {
            box:
                ...
                '<li class="fileuploader-input">' +
                '<button type="button" class="fileuploader-input-inner">' +
                '<i class="fileuploader-icon-main"></i> <span>${captions.feedback}</span>' +
                '</button></li></ul></div>',
            ...
            item:
                ...
                '<button type="button" class="fileuploader-action fileuploader-action-remove" title="${captions.remove}">' +
                '<i class="fileuploader-icon-remove"></i></button>' +
                ...
                '</div></div>' +
                '<div class="thumbnail-holder">${image}<span class="fileuploader-action-popup"></span>' +
            ...
            item2:
                ...
                '<a class="gallery-action-asmain">${captions.setting_asMain}</a>' +
                '</div></div>' +
                '<div class="thumbnail-holder">${image}<span class="fileuploader-action-popup"></span></div>' +
                '<div class="content-holder"><h5 title="${name}">${name}</h5><span>${size2}</span></div>' +
                '<div class="type-holder">${icon}</div>' +
                '</div></li>',
            ...
            canvasImage: true, // ✅ allow video frame drawing
            onItemShow: function (item, listEl, parentEl, newInputEl, inputEl) {
                ...
                item.html.find('.type-holder .fileuploader-item-icon')
                    [api.assets.isBrightColor(color) ? 'addClass' : 'removeClass']('is-bright-color')
                    .css('backgroundColor', color);
            },

            // ✅ UPDATED — adds video thumbnail & safe fallback
            onImageLoaded: function (item, listEl, parentEl, newInputEl, inputEl) {
                var api = $.fileuploader.getInstance(inputEl);
                var format = item.format;

                item.image.find('.fileuploader-item-icon i')
                    .html('')
                    .addClass('fileuploader-icon-' + (['image', 'video', 'audio'].indexOf(format) > -1 ? format : 'file'));
                ...
                // 🟢 handle small image check
                if (format === 'image' && item.upload && !item.imU) {
        ...
        upload: {
            ...
            onSuccess: function (result, item) {
                ...
                if (data.isSuccess && data.files.length) {
                    ...
                    item.name = file.name;
                    item.size = file.size;
                    item.size2 = file.size2;
                    item.data.url = file.url;
                    item.data.listProps.id = file.id;
            ...
            onError: function (item) {
                ...
                if (item.upload.status != 'cancelled' && !item.imU && !item.html.find('.fileuploader-action-retry').length) {
                    item.html.find('.actions-holder').prepend(
                        '<button type="button" class="fileuploader-action fileuploader-action-retry" title="Retry">' +
                        '<i class="fileuploader-icon-retry"></i></button>'
                    );
                }
            },
        ...
        onRemove: function (item) {
            if (item.data.listProps) {
                var name = item.data.listProps.original ? item.data.listProps.original : item.name;
